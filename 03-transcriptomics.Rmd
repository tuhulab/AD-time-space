...
cibersort <- 
  readr::read_csv("../cibersortx-deconvolute/data/multiomics-ad/emmaguttman_all_multiomics_ad.csv")[,1:11]

cibersort_stat <- 
  cibersort %>% 
  pivot_longer(cols = !Mixture, names_to = "cell_type", values_to = "Percentage") %>% 
  mutate(skin_type = Mixture %>% str_extract("LS|NL|NN"),
         Percentage = Percentage * 100) %>% 
  group_by(cell_type, skin_type) %>% 
  summarize(mean_per = mean(Percentage))

cibersort_stat %>% 
  ggplot(aes(x = skin_type, y = mean_per, fill = cell_type)) +
...
se_deconvolute_l %>%
  mutate(cell_type = cell_type %>% str_remove("cibersort..")) %>%
  ggplot(aes(x = sample, y = proportion, fill = cell_type)) +
  geom_bar(stat = "identity") +
  facet_wrap(~skin_type, scales = "free") +
  custom_theme + theme(axis.text.x = element_blank())

# se_deconvolute_l %>% 
#   mutate(cell_type = cell_type %>% str_remove("cibersort..")) %>%
#   filter(subject == "AD_02") %>% 
#   ggplot(aes(x = sample, y = proportion, fill = cell_type)) +
#   geom_bar(stat = "identity") +
#   facet_wrap(visit~skin_type, scales = "free") +
#   custom_theme + theme(axis.text.x = element_blank())
```
