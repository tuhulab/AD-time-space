...
```{r KRTAP}
se_KRTAP <- se %>% scale_abundance()
KRTAP_index <- se_KRTAP %>% rownames() %>% str_detect("KRTAP")
...
PCA %>% 
  ...
  ggplot2::scale_color_manual(values = pca_color) +
  theme_classic()
```

```{r PCA 3d plot, eval=FALSE}
attr(PCA, "internals")$PCA %>% 
  pca3d::pca3d(group = PCA$skin_type, col = pca_color, radius = 1.5, shape = "s", show.plane = FALSE)
...
```{r cleaning}
rm(list = ls()[!ls() %in% c("se", "se_KRTAP")])
```
...
heatmap <- ComplexHeatmap::Heatmap(heatmap_data_m, 
                        ...
                        name = "log(gene expression)", column_dend_reorder = FALSE,
...
png("figure/heatmap.png", width = 480*8*1.5, height = 480*10*1.5, res=300)
draw(heatmap, heatmap_legend_side = "right",
     annotation_legend_side = "top", merge_legend = FALSE)
dev.off()
...
```{r box plot for IL37, IL22}
IL22_37 <- se_space %>% filter(feature %in% c("IL22", "IL37"))

IL22_37 %>% as_tibble() %>% 
  ggboxplot(x = "skin_type", y = "counts_scaled", add = "jitter", 
            facet.by = "feature", scale = "free", xlab = FALSE,
            ylab = FALSE, color = "skin_type", palette = c("green", "orange", "red"), legend.title = " ",
            font.legend = c(14))
```
...
# gene rank
rank <- 
  tibble(gene_name = names(se_v1_LSvsNL)) %>% 
  left_join(de_v1_LSvsNL %>% pivot_transcript() %>% dplyr::select(gene_name = feature, WaldStatistic_skin_type_LS_vs_NL)) %>% 
  left_join(de_v1_LSvsNN %>% pivot_transcript() %>% dplyr::select(gene_name = feature, WaldStatistic_skin_type_LS_vs_NN)) %>% 
  left_join(de_v1_NLvsNN %>% pivot_transcript() %>% dplyr::select(gene_name = feature, WaldStatistic_skin_type_NL_vs_NN)) %>% 
  left_join(de_v1_LSvsNL_result %>% mutate(rank_log2fc_LSvsNL = log2FoldChange * -log10(padj)) %>% dplyr::select(gene_name, rank_log2fc_LSvsNL)) %>% 
  left_join(de_v1_LSvsNN_result %>% mutate(rank_log2fc_LSvsNN = log2FoldChange * -log10(padj)) %>% dplyr::select(gene_name, rank_log2fc_LSvsNN)) %>% 
  left_join(de_v1_NLvsNN_result %>% mutate(rank_log2fc_NLvsNN = log2FoldChange * -log10(padj)) %>% dplyr::select(gene_name, rank_log2fc_NLvsNN)) %>% 
...
```{r}
# TL_KRTAP_gene <- 
#   readxl::read_excel("/Users/tuhu/Downloads/KRTAP_cross_annotation_v2.xlsx") %>% 
#   filter(`Gene name...1` %in% c(paste0("KRTAP5-", 1:11), "KRTAP10-4"))

KRTAP_hair <-
  se_KRTAP %>% filter(feature %in% c(paste0("KRTAP5-", 1:11), "KRTAP10-4")) %>% 
  group_by(feature) %>% summarise(mean_expr = mean(counts), sd_expr = sd(counts)) %>% 
...
se_KRTAP_filt %>% 
  ggplot(aes(gender, log2(counts_scaled))) +
  geom_point() +
  geom_boxplot() +
  facet_wrap(~feature)
...
se_KRTAP_sgc <- 
  se_KRTAP_filt %>% left_join(
  multiomics_results %>% 
  filter(cell_type == "Sweat_gland_cells") %>% 
  select(BAM_ID, prop) %>% 
  rename(Sweat_gland_cells = prop)) %>% 
  filter(!is.na(Sweat_gland_cells))

se_KRTAP_sgc %>% assay(2) %>% 
  rbind(sgc = se_KRTAP_sgc$Sweat_gland_cells)  %>% 
  t() %>% 
  coop::pcor()
