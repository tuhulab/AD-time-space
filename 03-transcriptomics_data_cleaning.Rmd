...
gse_res_filt <- 
  ...
  filter(pathway %in% c("PID_IL12_2PATHWAY",
                        "PID_IL4_2PATHWAY",
...
gse_res_filt %>% 
  ggplot(aes(contrast, pathway, size = log10p, color = NES)) + 
  geom_point() +
  scale_size_area(max_size = 5) + 
  scale_color_gradient(low="pink", high="red") +
  theme_bw() +
  facet_grid(gs~., scales = "free")
```

```{r}
gse_res %>% group_by(gs) %>% summarise(n = n())

```

```{r card short description}
gse_res_filt_query <-
  gse_res_filt %>% select(pathway) %>% 
  distinct() %>% 
...
brief_description <-
  lapply(gse_res_filt_query$url, function(url){
    url %>% xml2::read_html() %>% 
      rvest::html_node(css = "tr:nth-child(3) td") %>% rvest::html_text()}
)
