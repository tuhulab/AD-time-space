...
dge_time_alone <-
  ...
  mutate(se = map(skin_type, ~ se[, se$skin_type == .x]),
         f = paste("~", t),
         deseq = map2(se, f, ~ DESeqDataSet(.x, design = .y %>% as.formula)),
         deseq = map(deseq, ~ DESeq(.x, test = "LRT", reduced = ~ 1)),
         deseq = map(deseq, ~ results(.x) %>% as_tibble(rownames = "gene_name")))
...
dge_time_result <-
  lapply(1:nrow(dge_time_design), function(index){
    se <- se[,se$skin_type == dge_time_design$skin_type[index]]
    f <- paste("~", dge_time_design$t[index])
...
names(dge_time_result) <- dge_time_design$name

dge_time_result_wN <-
  map2_dfr(dge_time_result,
     dge_time_design$name,
     ~ mutate(.x, name = .y) %>% 
       filter(padj < .05, abs(log2FoldChange) > 1)) 

g_t <- dge_time_result_wN$gene_name %>% unique
writeLines(g_t, "data/enrichr_gt.txt")
...
# rm(list = ls()[ls() != "se"])
# LS vs NL
se_v_LSvsNL <- se %>% filter(skin_type %in% c("LS", "NL"))
de_v_LSvsNL <- DESeq2::DESeqDataSet(se_v_LSvsNL, ~ subject + visit + skin_type + skin_type * visit)

de_v_LSvsNL_m <- DESeq2::DESeq(de_v_LSvsNL, test="LRT", 
...
de_v1_LSvsNL_LFC %>% as.data.frame() %>% tibble::rownames_to_column(var = "gene_name") %>% tibble()
