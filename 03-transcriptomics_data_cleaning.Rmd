...
dge_tstat_result <-
  plyr::llply(1:nrow(dge_tstat_design), function(index){
    ...
    deseq <- DESeq(deseq, test = "LRT", reduced = ~ 1, 
                   parallel = T, BPPARAM = MulticoreParam(6))
    deseq <- results(deseq)
...
dge_tstat_result_filt <-
  map2_dfr(dge_tstat_result,
           ~ filter(.x, padj < .05, abs(log2FoldChange) > 1)) 

dge_tstat_g <- dge_time_result_filt$gene_name %>% unique
...
## Differential expressed genes: time (NEW)
```{r load dge-time}
dge_time <- 
...
dge_time[[2]] %>% unnest(cols = "result") %>% 
  filter(padj < .05, abs(log2FoldChange) > 1) %>% 
  group_by(t, contrast, result_n) %>% summarise(n = n())


dge_time[[2]] %>% unnest(cols = "result") %>% 
  filter(padj < .05, abs(log2FoldChange) > 1) %>% 
  filter(t == "visit") %>% pull(gene_name) %>% unique()
...
enriched <- 
  enrichr(dge_time[[2]] %>% unnest(cols = "result") %>% 
  filter(padj < .05, abs(log2FoldChange) > 1) %>% 
  filter(t == "visit") %>% pull(gene_name) %>% unique(), 
  dbs)
