...
test <- gsea_exp %>% select(contrast, gs_name, project, rank_n)

gse_exp <- 
  expand.grid(rank = names(ls_gsa) %>% str_remove("de_v1_"), 
              gs = names(gs_list)) %>% 
  tibble() %>% 
  mutate(name = paste(gs, rank, sep = "_"), 
