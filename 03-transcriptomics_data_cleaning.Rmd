...
```{r gsa-prepare-data, cache=TRUE}
gse121212 <- readr::read_rds("../gse121212/data/DGE_full_model.rds")
gse121212_l <- gse121212$res_shrink
names(gse121212_l) <- gse121212$contrast

ls_gsa <- 
  c(de_v1_full, gse121212_l) %>% 
...
```{r fgsea, cache=TRUE}
gse_exp <- 
  expand.grid(rank = names(ls_gsa) %>% str_remove("de_v1_"), gs = names(gs_list)) %>% 
  tibble() %>% mutate(name = paste(gs, rank, sep = "_"), 
                      project = case_when(rank %in% c("LSvsNL", "LSvsNN", "NLvsNN") ~ "multi-omics",
                                          TRUE ~ "gse121212"))

gse_res <- purrr::map2_dfr(gse_exp$rank,
                           gse_exp$gs,
            function(rank, gs){
  gsa_stat <- fgsea::fgsea(pathways = gs_list[[gs]], 
                           ...
                           minSize  = 8,
                           maxSize  = 500, eps = 0,
                           BPPARAM = MulticoreParam(future::availableCores() - 4)) %>% 
    as_tibble() %>% 
    mutate(name = paste(gs, rank, sep = "_"), gs = gs, contrast = rank)
    # filter(padj < .05)
}) %>% 
  mutate(project = case_when(contrast %in% c("LSvsNL", "LSvsNN", "NLvsNN") ~ "multi-omics",
                             TRUE ~ "gse121212"))
...
gse_res_filt <-
  ...
  filter(pathway %in% c("KEGG_CHEMOKINE_SIGNALING_PATHWAY",
           ...
           pathway %>% str_detect("REACTOME_INTERLEUKIN")
         ) %>% 
  group_by(pathway, gs) %>% nest() %>% 
  ...
  mutate(cat = case_when(
    ...
    pathway %in% c("GOBP_KERATINOCYTE_DIFFERENTIATION",
                   "GOBP_REGULATION_OF_LIPID_BIOSYNTHETIC_PROCESS") ~ "skin barrier function",
