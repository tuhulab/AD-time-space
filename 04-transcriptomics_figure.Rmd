...
# ggsave("figure/gsea_ad.png", gsea_ad_p, width = 8, height = 7)
```
...
gsea_r_p <- 
  ...
  scale_color_gradient2(low="blue", mid = "white", high="red") +
  theme_bw() +
  facet_grid(gs_name~project, scales = "free", space = "free") +
...
multiomics_statics <-
  ...
  mutate(across(!BAM_ID, ~ ifelse(.x == 0, NA, .x)),
         across(!c("BAM_ID", "Keratinocytes"), ~ log(.x / Keratinocytes)))  %>% 
  left_join(colData(se_scaled) %>% as_tibble() %>% select(BAM_ID, group, subject, visit, skin_type, replicate_ID, gender, scorad, scorad_objective, easi_total_score, date_visit, rna_quality, visit_quarter, biopsy_area))
                # ~ log(.x / Keratinocytes)))

summary(aov(`T-cells` ~ skin_type, 
            data=multiomics_statics))
...
se[rownames(se) %in% c("IL34", "IL37", cytokine_df$from),] %>% 
