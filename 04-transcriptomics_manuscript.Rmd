...
example_boxplot_heatmap <- 
  se[c("IL22", "IL37", "TNFRSF21", "LINC01605", "OSMR"),]
...
```{r}
top_rna_quality_g <-
  vp_all %>% as_tibble(rownames = "gene_name") %>% 
  slice_max(rna_quality, n = 10) %>% pull(gene_name)

se[top_rna_quality_g,] %>% 
  ggplot(aes(x = rna_quality, y = counts_scaled)) +
  geom_boxplot() + facet_wrap("feature", scales = "free")
```


```{r vp_vary_100_3000, eval=FALSE}

# vp_vary_100_3000 <- 
#   lapply(seq(from = 100, to = 3000, by = 100), function(top_gene){
#   se_vary <- se %>% keep_variable(top = top_gene)
#   vp_vary <- variancePartition::fitExtractVarPartModel(
#     assay(se_vary, 2), 
#     ~ (1|skin_type) + (1|subject) + (1|visit) + (1|gender), 
#     colData(se_vary) %>% as_tibble(), 
#     BPPARAM = MulticoreParam(future::availableCores() - 2)
#     )
#   return(vp_vary)
# })
# 
# lapply(vp_vary_100_3000[1:30], variancePartition::plotVarPart)
...
vp_core <- variancePartition::fitExtractVarPartModel(
    # assay(se_core, 2),
    vobj_core,
    # assay(se_core, 2) %>% t() %>% scale() %>% t() %>% log1p(),
    # scale(assay(se_core, 2) %>% t() %>% log1p(), 
      # center = TRUE, scale = TRUE) %>% t(),
    ~ (1|sequencing_batch) +
...
# names(vp_core)[1] <- "anatomic area"
# names(vp_core)[3] <- "sequencing batch"
# names(vp_core)[4] <- "tissue type (LS/NL/HC)"
# names(vp_core)[5] <- "individual"
# names(vp_core)[7] <- "quarter"
# names(vp_core)[8] <- "SCORAD"
# names(vp_core)[9] <- "EASI"
# names(vp_core)[10] <- "unexplained"

# vpcore_violin <-
#   variancePartition::plotVarPart(vp_core)

vp_core_t <- vp_core %>% as_tibble(rownames = "gene_name")
  # pivot_longer(!gene_name, names_to = "term", values_to = "var")

# vp_core_t %>% ggviolin(x = "term", y = "var")
